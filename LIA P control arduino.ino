/* The following program is designed to create a controller using proportional gain (P). The controller will output a 
signal to an LED, which will then be read by an LDR. The state of the LDR will be read as an analog input. The setpoint 
(Sp) will be what the light intensity of the LED should be and the LDR will output the process value (PV). Based on the 
results, the out put to the LED will be controlled using an analog output. The goalof the PI control will be to minimize 
error as much as possible and have it so that the PV follows the SP as closely as possible with very minimal to no error. 
The results and values will be displayed on the serial monitor, lcd and serialplotter. It will utlize several lcd screens,
which will be switched between each other between left right pushbuttons. Some variables on the screen will also be able 
to increase and decrease with up down pushbuttons. */
...
float Pgain= 0.01;   //set float Pgain (proportional gain) to 0.01
...
int Error= SetP-PV;           //set error to equal the SetP minus the PV
int CO= ((Pgain*Error)/10);     //CO is equal to Pgain times Error all divided by 10 
...
Serial.println(Pgain);                //serial print Pgain
...
delay(100);          //delay 100 miliseconds 
  ...
  if (UP == LOW) {                //if pushbutton up is pressed then               
    Pgain = Pgain + 0.01;         //Pgain counter adds +0.01 

  } if (DOWN == LOW) {           //if pushbutton down is pressed then
    Pgain = Pgain - 0.01;           //Pgain counter subtracts by 0.01
    if (Pgain <= 0) {          //if Pgain is less than or equal to 0.0 then
      Pgain = 0;               //Pgain equals 0.0
    } 
  }
